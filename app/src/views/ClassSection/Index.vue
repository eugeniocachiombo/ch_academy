...
import { useClassSectionStore } from '@/stores/classSection.store';
import ClassSectionDialog from '@/views/ClassSection/Dialog.vue';
import { useToast } from 'primevue/usetoast';
import { findModel } from '@/services/general.service.js';
import { useUserStore } from '@/stores/user.store.ts';


const classSectionStore = useClassSectionStore();
const toast = useToast();
const user = useUserStore();


const pluralLabel = 'Turmas';
const singularLabel = 'Turma';
...
const filteredItems = computed(() => {
  let list = classSectionStore.classSections || [];
...
async function handleSave(formPayload) {
  if (classSectionStore.isLoading) return;

  try {
    if (isEditing.value && selectedItem.value) {
      await classSectionStore.update(selectedItem.value.id, formPayload);
      isDialogVisible.value = false;
      toast.add({ severity: 'success', summary: 'Sucesso', detail: `${singularLabel} actualizado com sucesso.`, life: 3500 });
    } else {
      await classSectionStore.create(formPayload);
      isDialogVisible.value = false;
...
async function executeDelete() {
  if (!itemToDelete.value || classSectionStore.isLoading) return;

  try {
    await classSectionStore.remove(itemToDelete.value.id);
  ...
  <section class="category-page">
    <header class="page-heading">
      <div class="heading-content">
        <div class="title-with-badge">
          <h1>Turmas</h1>
          <Tag v-if="classSectionStore.classSections" :value="classSectionStore.classSections.length" severity="info" class="count-badge" />
        </div>
        <p>Gerencie {{pluralLabel.toLowerCase()}} do sistema.</p>
      </div>
      <div class="heading-actions">
        <Button
          v-if="user.getPermition('classSection.create')"
          label="Nova Turma"
          icon="pi pi-plus"
          class="p-button-primary btn-add"
          :disabled="classSectionStore.isLoading"
    ...
    <div class="kpi-summary-grid">
      <div class="kpi-card">
        ...
        <div class="kpi-details">
          ...
          <span class="kpi-value">{{ classSectionStore.classSections?.length || 0 }}</span>
        </div>
      </div>
      <div class="kpi-card">
        ...
        <div class="kpi-details">
          ...
          <span class="kpi-value">{{ filteredItems.length }} / {{ classSectionStore.classSections?.length || 0 }}</span>
        </div>
      </div>
      <div class="kpi-card">
    ...
    <Card class="table-card border-none shadow-1">
      <template #content>
        <DataTable
          :value="filteredItems"
          :loading="classSectionStore.isLoading"
            ...
            <div class="table-header-wrapper table-header">
              <div class="filter-top-row">
                ...
                <div class="toolbar-actions">
                  <Button icon="pi pi-refresh" class="p-button-text p-button-secondary p-button-rounded" :loading="classSectionStore.isLoading" @click="reloadData" />
                </div>
              </div>
            </div>
          </template>
          ...
          <Column header="Acções" style="width: 140px; text-align: center">
            <template #body="{ data }">
              <div class="action-buttons">
                <Button
                  v-if="user.getPermition('classSection.update')"
                  icon="pi pi-pencil"
                  class="p-button-text p-button-rounded p-button-warning action-btn"
                  @click="openEditDialog(data)"
                />
                <Button
                  v-if="user.getPermition('classSection.delete')"
                  icon="pi pi-trash"
    ...
    <ClassSectionDialog
      ...
      :isLoading="classSectionStore.isLoading"
