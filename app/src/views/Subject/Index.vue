...
import SubjectDialog from '@/views/Subject/Dialog.vue';
...
const pluralLabel = 'Disciplinas';
const singularLabel = 'Disciplina';
...
const filteredItems = computed(() => {
  let list = subjectStore.subjects || [];
...
async function handleSave(formPayload) {
  if (subjectStore.isLoading) return;

  try {
    if (isEditing.value && selectedItem.value) {
      await subjectStore.update(selectedItem.value.id, formPayload);
      isDialogVisible.value = false;
      toast.add({ severity: 'success', summary: 'Sucesso', detail: `${singularLabel} actualizado com sucesso.`, life: 3500 });
    } else {
      await subjectStore.create(formPayload);
      isDialogVisible.value = false;
...
async function executeDelete() {
  if (!itemToDelete.value || subjectStore.isLoading) return;

  try {
    await subjectStore.remove(itemToDelete.value.id);
    ...
    <header class="page-heading">
      <div class="heading-content">
        <div class="title-with-badge">
          <h1>Disciplinas</h1>
          <Tag v-if="subjectStore.subjects" :value="subjectStore.subjects.length" severity="info" class="count-badge" />
        </div>
        <p>Gerencie as {{pluralLabel.toLowerCase()}} do sistema.</p>
      </div>
      <div class="heading-actions">
        <Button label="Nova Disciplina" icon="pi pi-plus" class="p-button-primary btn-add" :disabled="subjectStore.isLoading" @click="openCreateDialog" />
      </div>
    </header>
    ...
    <div class="kpi-summary-grid">
      <div class="kpi-card">
        ...
        <div class="kpi-details">
          ...
          <span class="kpi-value">{{ subjectStore.subjects?.length || 0 }}</span>
        </div>
      </div>
      <div class="kpi-card">
        ...
        <div class="kpi-details">
          ...
          <span class="kpi-value">{{ filteredItems.length }} / {{ subjectStore.subjects?.length || 0 }}</span>
        </div>
      </div>
      <div class="kpi-card">
    ...
    <Card class="table-card border-none shadow-1">
      <template #content>
        ...
        >
            ...
            <div class="table-header-wrapper table-header">
              <div class="filter-top-row">
                ...
                <div class="toolbar-actions">
                  <Button icon="pi pi-refresh" class="p-button-text p-button-secondary p-button-rounded" :loading="subjectStore.isLoading" @click="reloadData" />
                </div>
              </div>
            </div>
          </template>
          ...
          <Column field="name" header="Nome da Disciplina" sortable>
            <template #body="{ data }">
              <span class="font-medium">{{ data.name }}</span>
            </template>
          </Column>

          <Column field="class_id" header="Classe" style="width: 150px" sortable>
            <template #body="{ data }">
              <Tag :value="findModel(classStore.classs, data.class_id)?.name ?? '---- ---- ----'" :severity="getStatusSeverity(data.class_id)" />
            </template>
          </Column>
