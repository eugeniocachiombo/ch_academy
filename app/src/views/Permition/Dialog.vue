...
<template>
  <Dialog
    ...
    :header="isEditing ? 'Editar Permissão' : 'Nova Permissão'"
    :modal="true"
  ...
  >
    <form @submit.prevent="handleSave" class="form-grid space-y-4" novalidate>
      <div class="field">
        <label for="permition-name" class="required-label font-medium text-sm">Nome da Permissão</label>
        <InputText
          id="permition-name"
          v-model="form.name"
          placeholder="Ex: view, create, read"
    ...
    <div class="flex items-start gap-4 py-2">
      ...
      <div class="space-y-1" v-if="itemToDelete">
        <p class="text-sm leading-relaxed">
          Tem a certeza que deseja eliminar o Permissão
