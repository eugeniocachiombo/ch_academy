...
import PermitionDialog from '@/views/Permition/Dialog.vue';
...
const pluralLabel = 'Permissões';
const singularLabel = 'Permissão';
...
const filteredItems = computed(() => {
  let list = permitionStore.permitions || [];
...
async function handleSave(formPayload) {
  if (permitionStore.isLoading) return;

  try {
    if (isEditing.value && selectedItem.value) {
      await permitionStore.update(selectedItem.value.id, formPayload);
      isDialogVisible.value = false;
      toast.add({ severity: 'success', summary: 'Sucesso', detail: `${singularLabel} actualizado com sucesso.`, life: 3500 });
    } else {
      await permitionStore.create(formPayload);
      isDialogVisible.value = false;
...
async function executeDelete() {
  if (!itemToDelete.value || permitionStore.isLoading) return;

  try {
    await permitionStore.remove(itemToDelete.value.id);
  ...
  <section class="category-page">
    <header class="page-heading">
      <div class="heading-content">
        <div class="title-with-badge">
          <h1>Permissões</h1>
          <Tag v-if="permitionStore.permitions" :value="permitionStore.permitions.length" severity="info" class="count-badge" />
        </div>
        <p>Gerencie {{pluralLabel.toLowerCase()}} do sistema.</p>
      </div>
      <div class="heading-actions">
        <Button label="Nova Permissão" 
          v-if="user.getPermition('permition.create')"
          icon="pi pi-plus" 
          class="p-button-primary btn-add" 
          :disabled="permitionStore.isLoading" 
    ...
    <div class="kpi-summary-grid">
      <div class="kpi-card">
        ...
        <div class="kpi-details">
          ...
          <span class="kpi-value">{{ permitionStore.permitions?.length || 0 }}</span>
        </div>
      </div>
      <div class="kpi-card">
        ...
        <div class="kpi-details">
          ...
          <span class="kpi-value">{{ filteredItems.length }} / {{ permitionStore.permitions?.length || 0 }}</span>
        </div>
      </div>
      <div class="kpi-card">
    ...
    <Card class="table-card border-none shadow-1">
      <template #content>
        <DataTable
          :value="filteredItems"
          :loading="permitionStore.isLoading"
            ...
            <div class="table-header-wrapper table-header">
              <div class="filter-top-row">
                ...
                <div class="toolbar-actions">
                  <Button icon="pi pi-refresh" class="p-button-text p-button-secondary p-button-rounded" :loading="permitionStore.isLoading" @click="reloadData" />
                </div>
              </div>
            </div>
          </template>
    ...
    <PermitionDialog
      ...
      :isLoading="permitionStore.isLoading"
