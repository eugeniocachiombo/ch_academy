...
const permition_idOptions = computed(()=>
  permitionStore.permitions.map((e)=> ({
    "label": e.name,
    "value": e.id
  }))
);
...
watch(
  ...
  (newValue) => {
    if (newValue && props.isEditing) {
      form.permition_id = newValue.permition_id || '';
...
<template>
  ...
  >
    <form @submit.prevent="handleSave" class="form-grid space-y-4" novalidate>
      <div class="field">
        <label for="permitionRole-permition_id" class="required-label font-medium text-sm">Permissão</label>
        <Dropdown
          id="permitionRole-permition_id"
          v-model="form.permition_id"
          :options="permition_idOptions"
          optionLabel="label"
          optionValue="value"
          filter
          showClear
          ...
          :class="{ 'p-invalid': !!fieldErrors.permition_id }"
          :disabled="isLoading"
        />
         <small v-if="fieldErrors.permition_id" class="p-error-message" role="alert"><i class="pi pi-exclamation-circle"></i><span>{{ getErrorMessage(fieldErrors.permition_id) }}</span></small>
      </div>

      <div class="field">
        <label for="permitionRole-role_id" class="required-label font-medium text-sm">Função</label>
        <Dropdown
          id="permitionRole-role_id"
    ...
    <div class="flex items-start gap-4 py-2">
      ...
      <div class="space-y-1" v-if="itemToDelete">
        <p class="text-sm leading-relaxed">
          Tem a certeza que deseja eliminar a Permissão desta Função?
        </p>
