...
<template>
  <Dialog
    ...
    :header="isEditing ? 'Editar Função' : 'Novo Função'"
    :modal="true"
  ...
  >
    <form @submit.prevent="handleSave" class="form-grid space-y-4" novalidate>
      <div class="field">
        <label for="role-name" class="required-label font-medium text-sm">Nome da Função</label>
        <InputText
          id="role-name"
          v-model="form.name"
    ...
    <div class="flex items-start gap-4 py-2">
      ...
      <div class="space-y-1" v-if="itemToDelete">
        <p class="text-sm leading-relaxed">
          Tem a certeza que deseja eliminar o Função
